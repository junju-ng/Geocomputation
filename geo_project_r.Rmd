...
LSOA_stats <- kable(LSOA_stats, caption="Crime Count Statistics by LSOA")%>% kableExtra::kable_styling(latex_options = "hold_position")
LSOA_stats
```
...
Ranking the LSOAs by crime density, we obtain the following rank table (Table \@ref(tab:top20)), showing the top 20 LSOAs with the highest crime rates (crime count per 1000 people). We select the City of London 001F (COL001F) to do our analysis as it has the highest crime density. We are also interested to investigate why this is so.   
...
kable(top_20, caption = '20 LSOAs with the highest crime rates')%>%kableExtra::kable_styling(latex_options = "hold_position")
```
...
```{r COL001Fmapwroads, echo=FALSE, fig.cap="City of London 001F LSOA Map with Roads (red dots being VSO crime locations)",out.width="70%", fig.align='center'}
## plot map of COL001F
tmap_mode("plot")
tm_shape(roads)+
...
kable(table(points_on_roads), caption = 'Points on the streets') %>% kableExtra::kable_styling(latex_options = "hold_position")
```
