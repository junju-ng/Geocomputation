...
A line graph of crime count by type and month (\@ref(fig:linegraph)) is plotted to examine temporal patterns in crime in London across the year. \@ref(fig:linegraph_noASBVSO) depicts a similar graph, without anti-social behaviour and violence and sexual offences, in order to better observe trends in the other crime types. 

From \@ref(fig:linegraph), it is evident that the frequency of some crime types varies throughout the year. For instance, reports of anti-social behaviour appear to peak in July and August. Burglary reports appear to increase between October and January, while bicycle theft reports appear to increase between May and October (\@ref(fig:linegraph_noASBVSO)).
...
```{r linegraph_noASBVSO, echo=FALSE, fig.cap="2017 London crime count by type and month (excluding anti-social behaviour and violence and sexual offences"}
...
Crime counts are also examined by LSOA, in order to study the distribution of crime in London. From \@ref(fig:lsoacrimecount) and \@ref(tab:LSOA_stats), it is evident that crime counts vary significantly between LSOAs. Few LSOAs have extremely high crime counts, while 50% of LSOAs have crime counts between 64 and 211. Consequently, the quantiles classification scheme will be used to map the distribution of crime counts across London. 
...
tmap_mode("view")
tmap_mode("plot")
tm_shape(LondonLSOA_transformed)+ tm_layout("Crime counts by LSOA") +tm_polygons(col="Freq", palette="Blues", style = "quantile", border.col='transparent', title = "Crime counts") 
```

Consequently, a crime rate map (Figure \@ref(fig:crimeratemap)), derived by taking the crime count divided by the resident population in the corresponding LSOA, are plotted, to obtain an estimate of crime density. No significant difference between the crime rate and crime count maps is observed.

These crime rate maps allow for the assessment of the risk of crime in each LSOA [@Brantingham1997]. However, it is to be noted that residential populations are used to calculate crime rate. Thus, regions with a low residential population (but high pedestrian population e.g. commercial areas) would have an inflated crime rate. 
...
#census[is.na(census$Freq)]<- 0 #set Freq = 0 for LSOAs with no crimes

census$freq_pop <-  census$Freq / census$total_pop * 1000 #crime per 1000 people

binpal <- colorBin("Blues", census$freq_pop, 3, pretty = FALSE)
...
pop_up_text = paste("Region: ", census$LSOA11NM, "<br>",
                          "Value: ", census$freq_pop, "<br>")

LSOA_crime_pop_map <- leaflet(census) %>%
  addProviderTiles(providers$CartoDB.Positron) %>%
  setView(lng = -0.118092, lat = 51.509865, zoom = 9)%>%
  addPolygons(stroke = FALSE, smoothFactor = 0.2, fillOpacity = 1,
...
tm_shape(census)+ tm_layout("Crime rates by LSOA") + tm_polygons(col="freq_pop", palette="YlOrRd", style = "quantile", border.col='transparent', title = "Crime rates")
...
tm_shape(vs_LSOA)+ tm_polygons(col="Freq", palette="YlOrRd", style = "quantile")
```
...
tm_shape(vs_census)+ tm_polygons(col="freq_pop", palette="YlOrRd", style = "quantile")
...
#plot vehiclemap
tmap_mode("view")
tm_shape(LondonLSOA_transformed_vehicle)+ tm_polygons(col="Freq", palette="Greens", style = "quantile", n=3)
```
...
tm_shape(vehicle_census)+ tm_polygons(col="freq_pop", palette="YlOrRd", style = "quantile")
